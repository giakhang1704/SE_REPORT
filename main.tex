...
\input{Content/prjContext}
\newpage
\input{Content/Stakeholders}
\input{Content/goal&scope}
\newpage
\subsection{Yêu cầu chức năng}
\input{Content/func_req}

\input{Content/nonfunc}
...
\subsubsection{Use-case ch0 toàn bộ hệ thống}
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{Image/s.png}
...
\begin{figure}[H]
    ...
    \label{fig:placeholder}
\end{figure}
\input{Content/uc_User}
\input{Content/uc_DieuPhoiChuongTrinh}

\newpage
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.8\textwidth]{Image/Usecase_SINHVIEN.drawio.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{Image/USECASE_TUTOR.drawio.png}
    \caption{Nhóm chức năng của Tutor}
    \label{fig:placeholder}
\end{figure}
\input{Content/uc_Tutor}
...
\input{Content/diagram_DangNhap}

\subsubsection{Quên mật khẩu}
\input{Content/diagram_QuenMK}

\subsection{Thiết lập lịch rảnh}
\input{Content/diagram_thietlaplichranh}
\subsubsection{Đặt lịch hẹn}
\input{Content/diagram_DatLichHen}

\subsubsection{Ghi nhận tiến độ}
\input{Content/diagram_GhiNhanTienDo}

\subsubsection{Đăng tải tài liệu}
\input{Content/diagram_DangTai}

\subsubsection{Phản hồi chất lượng buổi học}
\input{Content/diagram_PhanHoiChatLuong}

\subsubsection{Hủy buổi gặp mặt}
\input{Content/diagram_HuyBuoiGap}

\subsubsection{Xử lý lịch hẹn}
\input{Content/diagram_xulylichhen}

\subsubsection{Tạo buổi tư vấn}
\input{Content/diagram_TaoBuoiTuVan}

\subsubsection{Điều phối khung chương trình chung}
\input{Content/diagram_DP}

\subsection{Mockup}

\input{Content/mockup.tex}
...
\input{Content/state_YC}
\subsubsection{Thiết lập lịch rảnh}
\input{Content/state_ThietLapLichRanh}
\subsubsection{Đăng ký Tutor}
\input{Content/state_DangKyTutor}
\subsubsection{Đăng tải và xóa tài liệu}
\input{Content/state_DangTaiXoa}
% % %%%%%%%%%%%%%%
...
\input{Content/devview.tex}
\subsection{Class diagram}
\input{Content/classdiagram.tex}

\subsection{Test case}
\input{Content/testcase.tex}

\section{Task 4}
\subsection{Hiện thực}
\input{Content/hienthuc.tex}

\subsection{Luồng hoạt động của hệ thống}
\input{Content/hoatdong.tex}

\subsection{Về việc sử dụng AI}
\input{Content/AI.tex}

\section{Nhật ký thay đổi}

\begin{itemize}
    \item Cập nhật mô tả các use case cho phù hợp hơn với chức năng thực tế của hệ thống, đồng bộ với yêu cầu nghiệp vụ sau khi rà soát lại.
    
    \item Vẽ lại Use Case Diagram nhằm thể hiện đầy đủ các actor, luồng tương tác và phạm vi chức năng của hệ thống sau khi đã hoàn thiện phân tích yêu cầu.
    \item Cập nhật Mockup.

    
    \item Điều chỉnh Class Diagram theo kiến trúc MVC với 5 lớp (View, Controller, Service, Repository, Entity), thay đổi mô tả các phương thức trong Class Diagram cho khớp với phần hiện thực hoá bằng Spring Boot.
    
    \item Cập nhật lại Sequence Diagram để thể hiện đúng luồng xử lý từ giao diện React đến API Spring Boot.

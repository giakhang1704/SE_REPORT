...
\begin{center}
View $\rightarrow$ Controller (API) $\rightarrow$ Service ((Interface + Implementation)) $\rightarrow$ Repository (Interface + Implementation)

$\rightarrow$ Entity (Model)
\end{center}

So với MVC truyền thống (Model – View – Controller), phiên bản mở rộng bổ sung thêm hai tầng abstraction:
\begin{itemize}
    \item \textbf{Service Layer} (Interface + Implementation)
    \item \textbf{Repository Layer} (Interface + Implementation) 

\end{itemize}

Hệ thống được xây dựng theo mẫu thiết kế MVC mở rộng (Multi-layer MVC), trong đó mỗi nhóm lớp được tổ chức theo đúng vai trò của từng tầng trong kiến trúc. So với MVC truyền thống, hệ thống bổ sung thêm tầng Service và Repository để tách biệt rõ UI, điều phối, nghiệp vụ và truy xuất dữ liệu.


\begin{itemize}
    \item Ở tầng View, các lớp như AppointmentManagementUI, AppointmentBookingUI, StudentMeetingListUI, … chỉ chịu trách nhiệm hiển thị thông tin và nhận thao tác từ người dùng (chọn buổi hẹn, gửi yêu cầu hủy, xem chi tiết…). Tầng này không chứa bất kỳ logic nghiệp vụ nào, đảm bảo nguyên tắc Separation of Concerns.

    \item Tầng Controller như TutorSchedulingAPI, ... tiếp nhận request từ View/FE, thực hiện kiểm tra đầu vào cơ bản và chuyển tiếp yêu cầu xuống Service tương ứng. Controller hoàn toàn không xử lý nghiệp vụ mà chỉ đóng vai trò điều phối luồng dữ liệu giữa View và Service.

    \item Tầng Service là nơi chứa toàn bộ nghiệp vụ cốt lõi. Gồm interface như ITutorSchedulingService,.. và lớp triển khai như TutorSchedulingService,... Tại đây hệ thống xử lý các hành động chẳng hạn duyệt lịch hẹn (approveAppointment), từ chối lịch hẹn (rejectAppointment), hủy buổi gặp (cancelMeeting), xem chi tiết (viewMeetingDetails, viewAppointmentDetails)… Việc Service tách thành interface + implementation giúp controller không phụ thuộc vào cách nghiệp vụ được cài đặt, dễ mở rộng và dễ thay thế.

    \item Tầng Repository gồm interface như  IMeetingRepository,... và lớp triển khai như MeetingRepository,...  chịu trách nhiệm cung cấp dữ liệu cho Service. Repository lưu trữ danh sách Meeting/Appointment/Consultation dưới dạng bộ nhớ tạm (vì phiên bản này không dùng DB). Tầng này mô phỏng việc truy xuất dữ liệu như một kho dữ liệu thực sự, giúp Service không cần biết dữ liệu được lấy từ đâu.

    \item Cuối cùng, tầng Entity (Model) gồm các lớp dữ liệu như Meeting, Appointment, Consultation,...  Đây là các cấu trúc đối tượng thuần, dùng để biểu diễn thông tin buổi hẹn và được luân chuyển giữa các tầng.
\end{itemize}

Việc áp dụng MVC mở rộng trong dự án giúp hệ thống phân tách rõ ràng vai trò từng lớp, giảm sự phụ thuộc giữa các tầng, thuận lợi cho bảo trì, mở rộng chức năng và kiểm thử. Kiến trúc này đồng thời phản ánh đúng cách phát triển ứng dụng hiện đại theo chuẩn Spring Boot.
...
Ngoài ra, mẫu thiết kế Facade được thể hiện thông qua lớp \texttt{TutorRegistrationFacade}.
Lớp này đóng vai trò như một điểm truy cập duy nhất, cung cấp một giao diện đơn giản cho
phía client (controller) khi làm việc với nhiều service phức tạp bên dưới. Thay vì để controller
trực tiếp tương tác và phối hợp với nhiều lớp xử lý khác nhau, TutorRegistrationFacade chịu
trách nhiệm tập trung các thao tác liên quan đến đăng ký, hủy đăng ký và chấp nhận yêu cầu
tư vấn. 
...
\textbf{Nhận diện Strategy Pattern}\\
Trong hệ thống, \texttt{SlotOperationStrategy} được sử dụng như một \textbf{Strategy Pattern}.  
Mỗi loại nghiệp vụ xử lý slot (ghi đè – đặt – huỷ) được đóng gói thành một chiến lược riêng, giúp \texttt{FreeSlotService} tách biệt logic và dễ mở rộng.

\textbf{Cấu trúc áp dụng Strategy}
\begin{itemize}
    \item \textbf{Interface Strategy:} \texttt{SlotOperationStrategy}
    \item \textbf{Concrete Strategies:}
    \begin{itemize}
        \item \texttt{OverwriteStrategy}
        \item \texttt{ReserveStrategy}
        \item \texttt{ReleaseStrategy}
    \end{itemize}
    \item \textbf{Context:} \texttt{FreeSlotService}
\end{itemize}

\textbf{Vai trò Client / Context}  
\texttt{FreeSlotService} đóng vai trò là Context, lựa chọn strategy phù hợp và gọi \texttt{execute()} mà không cần biết chi tiết xử lý.

\textbf{Interface: SlotOperationStrategy}\\
\begin{itemize}
    \item \texttt{execute(tutorId: Long, request: FreeSlotRequest): void}  
    -- Thực hiện nghiệp vụ xử lý slot tùy theo strategy được chọn.
\end{itemize}

\textbf{Chi tiết các Concrete Strategy}
\begin{itemize}
    \item \textbf{OverwriteStrategy}
    \begin{itemize}
        \item \texttt{execute(...)} -- Ghi đè slot, xử lý va chạm và cập nhật dữ liệu.
        \item \texttt{mergeInputRanges(...)} -- Gộp các khoảng thời gian liền kề hoặc chồng lấn.
        \item \texttt{splitRangeByAppointments(...)} -- Cắt slot rảnh lớn thành các slot nhỏ dựa trên lịch hẹn.
    \end{itemize}

    \item \textbf{ReleaseStrategy}
    \begin{itemize}
        \item \texttt{execute(...)} -- Chuyển slot đã đặt thành rảnh.
        \item \texttt{mergeTutorSlots(...)} -- Hợp các slot rảnh liền nhau.
    \end{itemize}

    \item \textbf{ReserveStrategy}
    \begin{itemize}
        \item \texttt{execute(...)} -- Tìm slot rảnh phù hợp và cắt phần thời gian bận ra khỏi slot gốc.
    \end{itemize}
\end{itemize}

\textbf{Lý do sử dụng Strategy Pattern}
\begin{enumerate}
    \item \textbf{Tách biệt logic phức tạp:}  
    Mỗi nghiệp vụ xử lý slot được viết trong class riêng, giúp code rõ ràng và dễ theo dõi.
    
    \item \textbf{Tuân thủ Open/Closed Principle:}  
    Khi thêm nghiệp vụ mới (ví dụ “Khoá lịch tạm thời”), chỉ cần tạo strategy mới mà không chỉnh sửa lớp \texttt{FreeSlotService}.
    
    \item \textbf{Dễ bảo trì và mở rộng:}  
    Client chỉ gọi \texttt{execute()} từ interface, không quan tâm chi tiết xử lý.
\end{enumerate}
...
\textbf{ForgotPasswordUI}
    \begin{itemize}
        \item displayForgotPasswordForm(): void -- Form nhập ID/email để reset.
        \item displayResetPasswordForm(token: String): void -- Form nhập mật khẩu mới.
        \item showPasswordResetSuccessMessage(): void -- Thông báo reset thành công.
        \item showErrorMessage(msg: String): void -- Thông báo lỗi.
        \item onRequestResetClick(callback): void -- Xử lý sự kiện gửi yêu cầu reset.
        \item onResetPasswordClick(callback): void -- Xử lý sự kiện lưu mật khẩu mới.
    \end{itemize}

\textbf{UserProfileUI}
    \begin{itemize}
        \item onOpenProfile(): void -- Mở trang cá nhân, gọi API lấy dữ liệu.
        \item displayProfile(user: UserEntity): void -- Hiển thị thông tin chi tiết người dùng.
        \item showErrorMessage(msg: String): void -- Hiển thị thông báo lỗi.
    \end{itemize}
...
\textbf{ListMeetingUI}
\begin{itemize}
    \item fetchOfficialMeetings(studentId: int): void\\
    Lấy danh sách các buổi học của sinh viên từ.
    \item formatMeetingTime(start: String, end: String): String\\
    Xử lý chuỗi thời gian thô thành định dạng hiển thị dễ đọc (Giờ - Ngày)
\end{itemize}

\textbf{FeedbackUI}
\begin{itemize}
    \item fetchMeetingInfo(id: int): void\\
    Lấy thông tin chi tiết (chủ đề, thời gian) của buổi hẹn hiện tại dựa trên ID.
    \item fetchFeedbackHistory(meetingId: int): void\\
    Tải lịch sử các phản hồi trước đó của sinh viên cho buổi hẹn này.
    \item handleRatingChange(rating: int): void\\
    Cập nhật số sao đánh giá của sinh viên
    \item handleCommentChange(text: String): void\\
    Cập nhật nội dung nhận xét khi sinh viên nhập liệu vào ô văn bản.
    \item handleSubmitFeedback(): void\\
    Kiểm tra dữ liệu (bắt buộc rating) và gửi đánh giá lên server qua API POST.
    \item navigateBack(): void\\
    Điều hướng quay trở lại trang danh sách buổi hẹn trước đó.
\end{itemize}
...
\textbf{UserProfileUI}
\begin{itemize}
    \item \textbf{onOpenProfile(): void} \\
    Xử lý sự kiện mở trang cá nhân và gọi Controller lấy dữ liệu.
    \item \textbf{displayProfile(user: UserEntity): void} \\
    Hiển thị thông tin chi tiết người dùng lên giao diện.
    \item \textbf{showErrorMessage(msg: String): void} \\
    Hiển thị thông báo lỗi khi không tải được dữ liệu hồ sơ.
\end{itemize}
...
\textbf{TutorViewFreeSlotsUI}
    \begin{itemize}
        \item fetchDailySlots(date: String): void -- Lấy danh sách khung giờ rảnh chi tiết.
        \item handleCalendarSelect(date: String): void -- Cập nhật state khi chọn ngày trên lịch.
        \item MapsToSetupView(): void -- Chuyển sang màn hình "Thiết lập lịch rảnh".
    \end{itemize}
\textbf{TutorUpdateFreeSlotUI}
    \begin{itemize}
        \item handleAddSlot(): void -- Thêm dòng nhập liệu mới.
        \item handleRemoveSlot(index: int): void -- Xóa dòng nhập liệu tại vị trí index.
        \item handleChangeTime(index: int, field: String, value: String): void -- Cập nhật giờ bắt đầu/kết thúc.
        \item handleSave(): void -- Thu thập dữ liệu và gọi API POST để lưu lịch.
        \item MapsToCalendarView(): void -- Quay lại màn hình xem lịch.
    \end{itemize}

\textbf{TutorRegistrationUI}
\begin{itemize}
    \item \textbf{render(): void} \\
    Hiển thị giao diện đăng ký tutor cho người dùng.
    \item \textbf{onSelectTutor(tutor: any): void} \\
    Xử lý sự kiện khi người dùng chọn một tutor từ danh sách.
    \item \textbf{onConfirm(): void} \\
    Xử lý sự kiện khi người dùng xác nhận đăng ký, gửi request tới controller.
\end{itemize}
...
\begin{itemize}
    ...
    \item getAppointmentDetail(id: Long): Appointment\\
    Xem chi tiết một Appointment theo ID.
...
\textbf{FreeSlotAPI}
    \begin{itemize}
        \item getDaily(date: LocalDate): ResponseEntity<FreeSlotResponse> -- Lấy lịch rảnh chi tiết.
        \item getMonthly(month: int, year: int): ResponseEntity<List<FreeSlotResponse>> -- Lấy lịch tóm tắt cả tháng.
        \item overwriteSchedule(request: FreeSlotRequest): ResponseEntity<?> -- Ghi đè lịch rảnh.
    \end{itemize}
...
\textbf{FeedbackAPI}
\begin{itemize}
    \item submitFeedback(meetingId: Long, rating: int, comment: String): Feedback\\
    Nhận phản hồi đánh giá buổi học từ sinh viên
    \item getFeedbacks(meetingId: Long): List<Feedback> \\
    Trả về danh sách đánh giá của một buổi học theo Id
\end{itemize}
...
\textbf{ForgotPasswordAPI}
    \begin{itemize}
        \item requestPasswordReset(bkId: String): void -- Kiểm tra user, tạo token, gửi email.
        \item resetPassword(token: String, newPassword: String): void -- Xác thực token và cập nhật mật khẩu.
    \end{itemize}

\textbf{UserProfileManageAPI}
    \begin{itemize}
        \item viewProfile(): void -- Lấy ID người dùng từ session và gọi Service.
    \end{itemize}
...
\begin{itemize}
    ...
    Lấy ID người dùng từ Session hiện tại, gọi Service lấy thông tin và trả về View.
\end{itemize}

\textbf{ProfileService}
    \begin{itemize}
        \item getUserProfile(userId: Long): UserEntity -- Lấy dữ liệu người dùng từ Repository.
    \end{itemize}
\textbf{SessionManager}
    \begin{itemize}
        \item createSession(user: UserEntity): String -- Tạo session mới.
        \item getSession(sessionId: String): SessionEntity -- Lấy thông tin session.
        \item invalidateSession(sessionId: String): void -- Hủy session.
        \item isValid(sessionId: String): Boolean -- Kiểm tra tính hợp lệ session.
    \end{itemize}

\textbf{TutorRegistrationController}
\begin{itemize}
    \item \textbf{registerTutor(request: RegisterRequest): ResponseEntity} \\
    Nhận request đăng ký tutor từ frontend và xử lý.
    \item \textbf{cancelRegistration(request: CancelRequest): ResponseEntity} \\
    Nhận request hủy đăng ký và xử lý.
    \item \textbf{suggestTutors(subject: String): ResponseEntity} \\
    Trả về danh sách tutor được gợi ý cho môn học cụ thể.
\end{itemize}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\end{itemize}
\textbf{FreeSlotService}
\begin{itemize}
    \item \textbf{getDailySchedule(tutorId: Long, date: LocalDate): FreeSlotResponse} \\
    Lấy danh sách Entity từ Repository, chuyển đổi sang DTO để trả về Controller.
    \item \textbf{getMonthlySchedule(tutorId: Long, month: int, year: int): List<FreeSlotResponse>} \\
    Lấy danh sách slot trong 1 tháng, gom nhóm theo ngày để trả về cấu trúc dữ liệu cho lịch tháng.
    \item \textbf{overwriteDailySchedule(tutorId: Long, request: FreeSlotRequest): List<String>} \\
    Xóa lịch cũ, tạo Entity mới, lưu lịch mới, trả về cảnh báo validate nếu có.
    \item \textbf{reserveSlot(tutorId: Long, date: LocalDate, start: LocalTime, end: LocalTime): void} \\
    Chuyển một slot từ trạng thái "Rảnh" sang "Đã đặt" khi có học viên đăng ký.
    \item \textbf{releaseSlot(tutorId: Long, date: LocalDate, start: LocalTime, end: LocalTime): void} \\
    Hủy trạng thái "Đã đặt", trả slot về lại trạng thái "Rảnh".
    \item \textbf{getRawAvailableSlots(tutorId: Long, date: LocalDate): List<TutorSlot>} \\
    Lấy trực tiếp danh sách Entity để phục vụ các xử lý nội bộ khác.
\end{itemize}
\textbf{RoomService}
\begin{itemize}
...
\begin{itemize}
    \item addFeedback(meetingId: Long, rating: int, comment: String): Feedback\\
    Kiểm tra và lưu phản hồi đánh buổi học của sinh viên
    \item getFeedbackByMeeting(meetingId: Long): List<Feedback>\\
    Lấy ra danh sách phản hồi đánh giá một buổi học theo ID buổi học
\end{itemize}
...
\textbf{TutorRegistrationService}
\begin{itemize}
    \item \textbf{createRequest(studentId: String, subject: String, tutorId: String): TutorRegistrationEntity} \\
    Tạo yêu cầu đăng ký tutor mới với trạng thái PENDING.
    \item \textbf{cancelRequest(registrationId: Long, studentId: String): boolean} \\
    Hủy yêu cầu đăng ký, kiểm tra quyền sở hữu trước khi hủy.
    \item \textbf{suggestTutor(subject: String): List<TutorSuggestion>} \\
    Lấy danh sách tutor phù hợp cho môn học (dựa trên rating, availability).
    \item \textbf{findPendingOlderThan(cutoff: LocalDateTime): List<TutorRegistrationEntity>} \\
    Tìm các yêu cầu PENDING cũ hơn một thời điểm để xử lý tự động.
    \item \textbf{approveRegistration(reg: TutorRegistrationEntity): void} \\
    Phê duyệt yêu cầu đăng ký (chuyển PENDING $\rightarrow$ APPROVED).
\end{itemize}
...
\textbf{UserRepository}
    \begin{itemize}
        \item findById(id: Long): UserEntity -- Lấy thông tin người dùng theo ID.
        \item createSession(user: UserEntity): String -- Tạo session mới.
        \item save(sessionId: String): void -- Lưu thay đổi session.
        \item update(sessionId: String): Boolean -- Cập nhật trạng thái session.
    \end{itemize}
\textbf{FreeSlotRepository}
\begin{itemize}
    \item \textbf{findAvailableByTutorIdAndDate(tutorId: Long, date: LocalDate): List<TutorSlot>} \\
    Lọc danh sách Mock để lấy các slot rảnh của gia sư trong ngày cụ thể.
    \item \textbf{findAvailableByTutorIdAndDateBetween(tutorId: Long, month: int, year: int): List<TutorSlot>} \\
    Tìm kiếm slot rảnh trong khoảng thời gian của tháng và năm.
    \item \textbf{saveAvailable(slots: List<TutorSlot>): void} \\
    Thêm danh sách các đối tượng TutorSlot mới vào kho lưu trữ.
    \item \textbf{findBookedByTutorIdAndDate(tutorId: Long, date: LocalDate): List<TutorSlot>} \\
    Lọc danh sách slot đã được học viên đặt.
    \item \textbf{saveBooked(slots: List<TutorSlot>): void} \\
    Lưu các slot đã chuyển sang trạng thái Booked vào kho lưu trữ.
    \item \textbf{deleteAllByTutorIdAndDate(tutorId: Long, date: LocalDate): void} \\
    Xóa toàn bộ slot của một ngày cụ thể để chuẩn bị cho việc ghi đè dữ liệu mới.
\end{itemize}
...
\begin{itemize}
    ...
    \item findByMeetingId(meetingId: Long): List<Feedback>\\
    Tìm danh sách phản hồi theo mã buổi học yêu cầu.
\end{itemize}
...
\textbf{MaterialRepository (Implementation)}
\begin{itemize}
    \item \textbf{save(m: MaterialEntity): MaterialEntity} \\
    Thực thi thao tác lưu tài liệu vào DB.
    \item \textbf{update(m: MaterialEntity): MaterialEntity} \\
    Thực thi cập nhật dữ liệu tài liệu trong DB.
    \item \textbf{getAllMaterials(): List<MaterialEntity>} \\
    Truy vấn toàn bộ bản ghi tài liệu từ DB.
    \item \textbf{getMaterialById(materialId: int): MaterialEntity} \\
    Thực thi câu lệnh tìm tài liệu theo ID.
    \item \textbf{getMaterialByUploader(tutorId: int): List<MaterialEntity>} \\
    Truy vấn danh sách tài liệu được tải lên bởi tutor.
    \item \textbf{searchMaterials(criteria: SearchCriteria): List<MaterialEntity>} \\
    Thực thi tìm kiếm tài liệu theo bộ tiêu chí.
    \item \textbf{getMaterialsByStatus(status: MaterialStatus): List<MaterialEntity>} \\
    Truy vấn tài liệu dựa vào trạng thái hiện tại.
\end{itemize}
...
\textbf{TutorRegistrationRepository}
\begin{itemize}
    \item \textbf{save(entity: TutorRegistrationEntity): TutorRegistrationEntity} \\
    Lưu hoặc cập nhật entity trong database.
    \item \textbf{findById(id: Long): Optional<TutorRegistrationEntity>} \\
    Tìm đăng ký theo ID.
    \item \textbf{findByStatus(status: TutorRegistrationStatus): List<TutorRegistrationEntity>} \\
    Tìm tất cả đăng ký theo trạng thái.
    \item \textbf{findByStudentIdAndStatus(studentId: String, status: TutorRegistrationStatus): List<TutorRegistrationEntity>} \\
    Tìm đăng ký của một học sinh với trạng thái cụ thể.
    \item \textbf{existsByStudentIdAndStatusIn(studentId: String, statuses: List<TutorRegistrationStatus>): boolean} \\
    Kiểm tra học sinh có đăng ký nào ở các trạng thái cho trước không.
    \item \textbf{findByRequestTimeBefore(cutoff: LocalDateTime): List<TutorRegistrationEntity>} \\
    Tìm đăng ký có thời gian request trước thời điểm cutoff.
\end{itemize}
...
\begin{itemize}
    ...
    Trả về Meeting \texttt{APPROVED} và chưa bị hủy của tutor.
\end{itemize}
\textbf{IFreeSlotRepository / FreeSlotRepository}
    \begin{itemize}
        \item findAvailableByTutorIdAndDate(...) -- Lấy slot rảnh trong ngày.
        \item findAvailableByTutorIdAndDateBetween(...) -- Lấy slot rảnh trong tháng.
        \item saveAvailable(...) -- Lưu slot rảnh mới.
        \item findBookedByTutorIdAndDate(...) -- Lấy slot đã đặt.
        \item saveBooked(...) -- Lưu slot đã đặt.
        \item deleteAllByTutorIdAndDate(...) -- Xóa toàn bộ slot của ngày.
    \end{itemize}

\textbf{IMaterialRepository}
\begin{itemize}
    \item \textbf{save(m: MaterialEntity): MaterialEntity} \\
    Lưu tài liệu mới vào cơ sở dữ liệu.
    \item \textbf{update(m: MaterialEntity): MaterialEntity} \\
    Cập nhật nội dung hoặc thông tin của một tài liệu đã tồn tại.
    \item \textbf{getAllMaterials(): List<MaterialEntity>} \\
    Trả về danh sách toàn bộ tài liệu.
    \item \textbf{getMaterialById(materialId: int): MaterialEntity} \\
    Lấy một tài liệu dựa trên ID.
    \item \textbf{getMaterialByUploader(tutorId: int): List<MaterialEntity>} \\
    Trả về danh sách tài liệu do một tutor cụ thể tải lên.
    \item \textbf{searchMaterials(criteria: SearchCriteria): List<MaterialEntity>} \\
    Tìm kiếm tài liệu dựa trên tiêu chí (tên, lớp, loại, từ khóa…).
    \item \textbf{getMaterialsByStatus(status: MaterialStatus): List<MaterialEntity>} \\
    Lấy danh sách tài liệu theo trạng thái (Draft, Pending, Published…).
\end{itemize}


\subsubsection{Service Interface Method}
\textbf{IFreeSlotService / FreeSlotService}
    \begin{itemize}
        \item getDailySchedule(tutorId: Long, date: LocalDate): FreeSlotResponse -- Lấy Entity và chuyển sang DTO.
        \item getMonthlySchedule(tutorId: Long, month: int, year: int): List<FreeSlotResponse> -- Gom nhóm slot theo ngày.
        \item overwriteDailySchedule(tutorId: Long, request: FreeSlotRequest): List<String> -- Xóa, tạo và lưu slot mới.
        \item reserveSlot(...) -- Chuyển slot từ Rảnh → Đã đặt.
        \item releaseSlot(...) -- Trả slot về trạng thái Rảnh.
        \item getRawAvailableSlots(...) -- Lấy danh sách Entity trực tiếp.
    \end{itemize}
...
\textbf{IMaterialUploadService}
\begin{itemize}
    \item \textbf{createDraft(tutorId: String, title: String, type: String, classId: String): Material} \\
    Tạo một tài liệu ở trạng thái nháp (draft) cho tutor trước khi upload nội dung.
    \item \textbf{attachFile(materialId: String, fileName: String, bytes: byte[]): Material} \\
    Đính kèm file (PDF, PPT, DOCX, hình ảnh…) vào tài liệu nháp.
    \item \textbf{submitForReview(materialId: String): void} \\
    Tutor gửi tài liệu sang trạng thái “chờ duyệt” để Admin/Tutor Leader xem và phê duyệt.
    \item \textbf{publish(materialId: String): void} \\
    Xuất bản tài liệu đã được duyệt để sinh viên có thể xem và tải về.
    \item \textbf{listMyUploads(tutorId: String): List<Material>} \\
    Trả về danh sách các tài liệu mà tutor đã tải lên (bao gồm draft, pending, published).
\end{itemize}
...
\textbf{IFeedbackService} 
\begin{itemize}
    \item addFeedback(meetingId: Long, rating: int, comment: String): Feedback\\
    Lưu phản hồi đánh buổi học 
    \item getFeedbackByMeeting(meetingId: Long): List<Feedback>\\
    Lấy ra danh sách phản hồi đánh giá một buổi học theo ID.
\end{itemize}

\textbf{ITutorRegistrationRepository}
\begin{itemize}
    \item \textbf{save(entity: TutorRegistrationEntity): TutorRegistrationEntity} \\
    Lưu hoặc cập nhật entity trong database.
    \item \textbf{findById(id: Long): Optional<TutorRegistrationEntity>} \\
...
\textbf{Feedback}
\begin{itemize}
    \item getFeedbackId(): Long\\
    Trả về mã đánh giá
    \item getMeetingId(): Long\\ 
    Trả về mã buổi học
    \item getRating(): int\\
    Trả về đánh giá (từ 1 đến 5)
    \item getComment(): String\\
    Trả về nhận xét đánh giá
    \item getSubmitAt(): LocalDateTime\\
    Trả về thời điểm đánh giá
\end{itemize}
...
\textbf{TutorSlot}
    \begin{itemize}
        \item isOverlapping(other: TutorSlot): boolean -- Kiểm tra trùng lặp slot.
    \end{itemize}
\textbf{UserEntity / SessionEntity} -- Chứa thông tin người dùng và phiên làm việc.
...
\begin{itemize}
    ...
    \item selectRoom(roomId: int): void\\
    Chọn một phòng để sử dụng trong buổi tư vấn.
\end{itemize}
...
\textbf{SchedulingAPI}
\begin{itemize}
    \item createConsultation(tutorId: int, date: Date, startTime: Time, endTime: Time, title: String, mode: String, maxParticipants: int, roomId: int, onlineLink: String): bool\\
    Tiếp nhận yêu cầu tạo buổi tư vấn từ UI, gọi SchedulingService.
    \item bookAppointment(studentId: int, consultationId: int, message: String): bool\\
    Tiếp nhận yêu cầu đặt lịch hẹn từ UI, gọi SchedulingService.
    \item approveAppointment(appointmentId: int, tutorId: int): bool\\
    Tiếp nhận yêu cầu duyệt lịch hẹn từ UI, gọi SchedulingService.
    \item rejectAppointment(appointmentId: int, tutorId: int): bool\\
    Tiếp nhận yêu cầu từ chối lịch hẹn từ UI, gọi SchedulingService.
    \item cancelMeeting(meetingId: int, userId: int, reason: String): bool\\
    Tiếp nhận yêu cầu hủy buổi gặp mặt từ UI, gọi SchedulingService.
    \item getMeetings(userId: int): List<Meeting>\\
    Trả về danh sách buổi gặp mặt của người dùng.
\end{itemize}
...
\begin{itemize}
    ...
    \item getRoomDetails(roomId: int): String\\
...
\textbf{SchedulingService}
\begin{itemize}
    \item createConsultation(tutorId: int, date: Date, startTime: Time, endTime: Time, title: String, mode: String, maxParticipants: int, roomId: int, onlineLink: String): bool\\
    Tạo một buổi tư vấn mới. Logic bao gồm: kiểm tra trùng lịch của tutor, kiểm tra phòng trống nếu offline, lưu thông tin vào Repository. Trả về true nếu tạo thành công, false nếu có lỗi.
    \item bookAppointment(studentId: int, consultationId: int, message: String): bool\\
    Đặt lịch hẹn với tutor dựa trên một buổi tư vấn có sẵn. Kiểm tra lịch rảnh của tutor, tránh trùng lịch. Trả về true nếu đặt thành công, false nếu lịch trùng hoặc lỗi.
    \item approveAppointment(appointmentId: int, tutorId: int): bool\\
    Duyệt một lịch hẹn đang ở trạng thái PENDING. Cập nhật trạng thái thành APPROVED và thêm vào danh sách Meeting của student và tutor. Trả về true nếu thành công.
    \item rejectAppointment(appointmentId: int, tutorId: int): bool\\
    Từ chối một lịch hẹn PENDING. Cập nhật trạng thái thành REJECTED. Trả về true nếu thao tác thành công.
    \item cancelMeeting(meetingId: int, userId: int, reason: String): bool\\
    Hủy một buổi gặp mặt (Appointment hoặc Consultation). Cập nhật trạng thái CANCELLED và lưu lý do hủy. Trả về true nếu hủy thành công.
    \item getMeetings(userId: int): List<Meeting>\\
    Lấy danh sách tất cả các buổi gặp mặt liên quan tới user (Student hoặc Tutor).
    \item \# validateScheduleConflict(date: Date, startTime: Time, endTime: Time, tutorId: int): bool\\
    Kiểm tra trùng lịch của tutor trong khoảng thời gian được yêu cầu.
    \item \# checkRoomAvailability(roomId: int, date: Date, startTime: Time, endTime: Time): bool\\
    Kiểm tra trạng thái phòng có trống hay không cho khoảng thời gian cụ thể. Phương thức private.
    \item \# checkTutorSlotAvailability(tutorId: int, date: Date, start: Time, end: Time): bool\\
    Kiểm tra xem slot của tutor có trống trong khoảng thời gian nhất định không.
\end{itemize}
...
\begin{itemize}
    ...
    Trả về danh sách phòng trống theo yêu cầu từ UI.
    \item updateRoomStatus(roomId: int, status: String): bool\\
    Cập nhật trạng thái phòng (AVAILABLE, OCCUPIED, MAINTENANCE). Trả về true nếu cập nhật thành công.
    \item getRoomDetails(roomId: int): String\\
    Lấy thông tin chi tiết của một phòng (tòa nhà, tầng, số phòng, sức chứa, trạng thái).
    \item getRoomStatistics(): Map<String, Any>\\
    Trả về thống kê tổng quan về các phòng: số phòng trống, số phòng đang sử dụng, số phòng bảo trì, phân theo tòa nhà hoặc tầng.
    \item \# isRoomAvailable(roomId: int, date: Date, startTime: Time, endTime: Time): bool\\
    Kiểm tra xem phòng có trống trong khoảng thời gian cụ thể hay không. Phương thức private.
\end{itemize}
...
\textbf{TutorSchedulingRepository}
\begin{itemize}
    \item createConsultation(tutorId: int, date: Date, startTime: Time, endTime: Time, title: String, mode: String, maxParticipants: int, roomId: int, onlineLink: String): bool\\
    Lưu buổi tư vấn mới vào cơ sở dữ liệu. Trả về true nếu tạo thành công.
    \item approveAppointment(appointmentId: int, tutorId: int): bool\\
    Duyệt lịch hẹn từ trạng thái PENDING → APPROVED. Trả về true nếu thành công.
    \item rejectAppointment(appointmentId: int, tutorId: int): bool\\
    Từ chối lịch hẹn, cập nhật trạng thái REJECTED.
    \item viewMeetings(userId: int): List<Meeting>\\
    Lấy danh sách tất cả các buổi gặp mặt liên quan đến Tutor.
    \item cancelMeeting(meetingId: int, userId: int, reason: String): bool\\
    Hủy một buổi gặp mặt, lưu lý do hủy. Trả về true nếu thành công.
\end{itemize}

\textbf{StudentSchedulingRepository}
\begin{itemize}
    \item registerConsultation(studentId: int, consultationId: int): bool\\
    Đăng ký sinh viên tham gia buổi tư vấn. Trả về true nếu thành công.
    \item bookAppointment(studentId: int, tutorId: int, date: Date, startTime: Time, topic: String): bool\\
    Đặt lịch hẹn với tutor. Trả về true nếu thành công.
    \item viewMeetings(userId: int): List<Meeting>\\
    Lấy danh sách tất cả các buổi gặp mặt liên quan đến Student.
    \item cancelMeeting(meetingId: int, userId: int, reason: String): bool\\
    Hủy một buổi gặp mặt, lưu lý do hủy. Trả về true nếu thành công.
\end{itemize}
...
\textbf{ITutorSchedulingRepository}
\begin{itemize}
    \item createConsultation(tutorId: int, date: Date, startTime: Time, endTime: Time, title: String, mode: String, maxParticipants: int, roomId: int, onlineLink: String): bool\\
    Lưu buổi tư vấn mới vào cơ sở dữ liệu. Trả về true nếu tạo thành công.
    \item approveAppointment(appointmentId: int, tutorId: int): bool\\
...
\textbf{IStudentSchedulingRepository}
\begin{itemize}
    \item registerConsultation(studentId: int, consultationId: int): bool\\
    Đăng ký sinh viên tham gia buổi tư vấn. Trả về true nếu thành công.
    \item bookAppointment(studentId: int, tutorId: int, date: Date, startTime: Time, topic: String): bool\\
...
\textbf{ISchedulingRepository}
\begin{itemize}
    \item viewMeetings(userId: int): List<Meeting>\\
    Lấy danh sách tất cả các buổi gặp mặt của người dùng (Tutor hoặc Student).
    \item cancelMeeting(meetingId: int, userId: int, reason: String): bool\\
    Hủy buổi gặp mặt, lưu lý do hủy. Trả về true nếu thành công.
\end{itemize}
...
\textbf{Room}
\begin{itemize}
    \item isAvailable(date, startTime, endTime): bool\\
    Kiểm tra phòng có trống.
    \item setStatus(newStatus): void\\
    Cập nhật trạng thái phòng.
    \item toString(): String\\
    Hiển thị thông tin phòng.
    \item Getter/Setter cho: roomId, building, floor, roomNumber, capacity, status.
\end{itemize}

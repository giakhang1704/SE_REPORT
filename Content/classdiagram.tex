...
\begin{itemize}
    \item \textbf{fetchDailySlots(date: String): void} \\
    Gọi API lấy danh sách các khung giờ rảnh chi tiết của ngày được chọn để hiển thị lên màn hình.
    \item \textbf{handleCalendarSelect(date: String): void} \\
    Xử lý sự kiện khi người dùng nhấn vào một ngày trên lịch, cập nhật state ngày hiện tại.
    \item \textbf{MapsToSetupView(): void} \\
    Chuyển giao diện sang màn hình "Thiết lập lịch rảnh" (Form nhập liệu).
\end{itemize}
\textbf{TutorUpdateFreeSlotUI}
\begin{itemize}
    \item \textbf{handleAddSlot(): void} \\
    Thêm một dòng nhập liệu (khung giờ mặc định) mới vào danh sách trên Form.
    \item \textbf{handleRemoveSlot(index: int): void} \\
    Xóa dòng nhập liệu khung giờ tại vị trí index khỏi Form.
    \item \textbf{handleChangeTime(index: int, field: String, value: String): void} \\
    Cập nhật giá trị giờ bắt đầu hoặc giờ kết thúc cho một dòng nhập liệu cụ thể.
    \item \textbf{handleSave(): void} \\
    Thu thập dữ liệu từ Form, đóng gói thành payload và gọi API POST để lưu lịch.
    \item \textbf{MapsToCalendarView(): void} \\
    Hủy thao tác hoặc quay trở lại màn hình xem lịch sau khi lưu thành công.
\end{itemize}
...
\textbf{FreeSlotAPI}
\begin{itemize}
    \item \textbf{getDaily(date: LocalDate): ResponseEntity<FreeSlotResponse>} \\
    API Endpoint (GET) nhận ngày, trả về thông tin lịch rảnh chi tiết.
    \item \textbf{getMonthly(month: int, year: int): ResponseEntity<List<FreeSlotResponse>>} \\
    API Endpoint (GET) trả về danh sách tóm tắt lịch rảnh của cả tháng.
    \item \textbf{overwriteSchedule(request: FreeSlotRequest): ResponseEntity<?>} \\
    API Endpoint (POST) nhận yêu cầu ghi đè lịch rảnh, trả về kết quả thành công hoặc danh sách cảnh báo lỗi.
\end{itemize}
...
\textbf{ProfileService / SessionManager}
\begin{itemize}
    \item \textbf{getUserProfile(userId: Long): UserEntity} \\
    Truy vấn Repository để lấy thông tin người dùng theo ID.
    \item \textbf{createSession(user: UserEntity): String} \\
    Tạo phiên làm việc mới, lưu thông tin người dùng và trả về SessionID.
    \item \textbf{getSession(sessionId: String): SessionEntity} \\
    Truy xuất thông tin chi tiết phiên dựa trên SessionID.
    \item \textbf{invalidateSession(sessionId: String): void} \\
    Hủy bỏ phiên làm việc khỏi hệ thống.
    \item \textbf{isValid(sessionId: String): Boolean} \\
    Kiểm tra tính hợp lệ và thời hạn của SessionID.
\end{itemize}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\end{itemize}
\textbf{UserRepository / SessionRepository}
\begin{itemize}
    \item \textbf{findById(id: Long): UserEntity} \\
    Truy vấn Database lấy thông tin người dùng theo khóa chính.
    \item \textbf{createSession(user: UserEntity): String} \\
    Lưu session mới vào kho dữ liệu và trả về SessionID.
    \item \textbf{save(sessionId: String): void} \\
    Lưu các thay đổi của session hiện tại.
    \item \textbf{update(sessionId: String): Boolean} \\
    Cập nhật trạng thái session và trả về kết quả thành công/thất bại.
\end{itemize}
...
\begin{itemize}
    ...
    Xóa toàn bộ slot của một ngày cụ thể để chuẩn bị cho việc ghi đè dữ liệu mới.
\end{itemize}
\subsection{Service Interface Method}
\textbf{IFreeSlotService}
\begin{itemize}
    \item \textbf{getDailySchedule(tutorId: Long, date: LocalDate): FreeSlotResponse} \\
    Lấy danh sách Entity từ Repository, chuyển đổi sang DTO để trả về Controller.
    \item \textbf{getMonthlySchedule(tutorId: Long, month: int, year: int): List<FreeSlotResponse>} \\

...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{Image/AC_SE-Ac Cancel Tutor.drawio.png}
    \caption{Activity Diagram cho Hủy buổi gặp mặt của tutor }
\end{figure}
...
\begin{enumerate}
\item Tutor truy cập vào buổi hẹn của tôi và chọn mục ``Hủy lịch''.
\item Hệ thống (UI) hiển thị danh sách buổi hẹn có thể hủy.
...
\item Hệ thống yêu cầu tutor xác nhận có muốn khôi phục lại lịch rảnh đã hủy không.
\item Hệ thống cập nhật trạng thái buổi gặp trong cơ sở dữ liệu (hủy buổi gặp) và cập nhật lại lịch rảnh của Tutor.
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Nếu Tutor không nhập nội dung phản hồi hoặc nội dung không đạt yêu cầu, Hệ thống hiển thị thông báo lỗi:
        \begin{quote}
            ``Nội dung phản hồi không hợp lệ, vui lòng nhập lại.''
        \end{quote}
        \item Sau đó, giao diện quay lại form phản hồi ban đầu (reset form rỗng).
        \item Tutor có thể chọn nhập lại nội dung và gửi lại hoặc nhấn hủy.
    \end{itemize}
